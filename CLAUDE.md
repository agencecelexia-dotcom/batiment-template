# Bâtiment - Site Web Professionnel

## Projet
Site web pour une entreprise de bâtiment tous corps d'état. Repository: `agencecelexia-dotcom/batiment-template`.
...
- Palette de couleurs : Terracotta/brun chaud (primary hue 30) + Vert professionnel (accent hue 145)
...
## Images — IMPORTANT
**Conserver les images existantes du template.** Le dossier `public/images/` contient des photos generiques pour le metier batiment. Lors de la personnalisation :
1. **NE PAS supprimer ni remplacer les photos existantes**
2. **NE PAS ajouter de nouvelles photos** — le client les ajoutera via le panel admin
3. Les chemins sont declares dans `src/config/images.ts`
4. Le composant `ImagePlaceholder` est utilise comme fallback
5. Le client peut remplacer les images via le panel admin

## SEO
- Metadata dynamiques par page
- Schema.org (GeneralContractor, Service, FAQPage)
...
- `{VILLE}`, `{COMMUNE_1}`, `{COMMUNE_2}`, `{COMMUNE_3}`
- `{ZONE_KM}`, `{NOM_ENTREPRISE}`, `{PRENOM_DIRIGEANT}`, etc.

**Workflow :** `CLIENT.md` → `npm run sync-client` → `src/config/client.config.ts` → tout le site se met a jour.
